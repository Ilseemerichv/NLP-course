...
Original file is located at
    https://colab.research.google.com/drive/1HYYOgPb2A5WdVTbCMHI64AFMyGoxFkzn
...
# Load the novel
#file_path = '/content/drive/MyDrive/EDC SEM 4/moby.txt'
...
def question_two():
    ...
    whale_count = sum(1 for t in tokens if t == 'whale' or t == 'Whale')
...
def question_six():
    ...
    # Fiter only word tokens
    words = [token for token in text1 if token.isalpha()]

    # Count frequency
    freq_dist = FreqDist(words)

    # Frequency > 2000
    filtered = [(freq, word) for word, freq in freq_dist.items() if freq > 2000]
...
def question_seven():
    ...
    """
    # YOUR CODE HERE
    sentences = sent_tokenize(moby_raw)  # divide text in sentences
    total_tokens = sum(len(word_tokenize(sent)) for sent in sentences)  # count tokens in sentences
    avg_tokens = total_tokens / len(sentences)  # average per sentence
...
def question_eight():
    ...
    # Filter alphabetical tokkens and stop words
    filtered_words = [token for token in text1 if token.isalpha() and token.lower() not in stop_words]
...
def question_nine():
    ...
    # Apply stemming (convert to lowercase to merge forms)
...
# NOTE: THE EXERCISE 9 DOES NOT MENTION REMOVING STOP WORDS, SO I KEPT THEM
