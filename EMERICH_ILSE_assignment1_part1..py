...
Original file is located at
    https://colab.research.google.com/drive/1eMkAuLZ_tq9Q6GFJ9GLlOaFr79CWDvwr
...
#Load the data
#doc = []
#with open('/content/drive/MyDrive/EDC SEM 4/dates.txt') as file:
#    for line in file:
#        doc.append(line)
...
def question_two():
    ...
    """
    # YOUR CODE HERE
    pattern = r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?[-\s]\d{1,2},?\s\d{4}"
...
def question_three():
    ...
    """
    # YOUR CODE HERE
    pattern = r"\b\d{1,2}\s(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s\d{4}"
...
def question_seven():
    ...
    # Regex patterns
    pattern_mmddyy = r'(\b\d{1,2}/\d{1,2}/\d{2}\b)'   # mm/dd/yy
    pattern_mmyyyy = r'(\b\d{1,2}/\d{4}\b)'           # mm/yyyy
    pattern_yyyy = r'(\b\d{4}\b)'                     # yyyy only

    for idx, note in df.items():  # Use your Series 'df'
        date_obj = None

        # mm/dd/yy
        match = re.search(pattern_mmddyy, note)
        if match:
            month, day, year = map(int, match.group(1).split('/'))
            year += 1900  # two-digit years are from 1900s
            date_obj = datetime(year, month, day)
        else:
            # mm/yyyy
            match = re.search(pattern_mmyyyy, note)
            if match:
                parts = list(map(int, match.group(1).split('/')))
                month = parts[0] if 1 <= parts[0] <= 12 else 1  # validate month
                year = parts[1]
                day = 1  # assume day = 1
                date_obj = datetime(year, month, day)
            else:
                # yyyy only
                match = re.search(pattern_yyyy, note)
                if match:
                    year = int(match.group(1))
                    date_obj = datetime(year, 1, 1)  # assume Jan 1st

        # If no date found, assign a far future date to sort at the end
        if date_obj is None:
            date_obj = datetime(9999, 1, 1)

        date_list.append((idx, date_obj))

    # Sort by date; if dates are equal, sort by original index to keep consistent order
    sorted_indices = [idx for idx, _ in sorted(date_list, key=lambda x: (x[1], x[0]))]

    return pd.Series(sorted_indices)  # Return the sorted indices
